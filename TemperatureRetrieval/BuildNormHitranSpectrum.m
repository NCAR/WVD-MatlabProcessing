...
function [Sigma] = BuildNormHitranSpectrum(Hitran,Nu,Pressure,Temperature)
%
% In
%
%
...
Constants = DefineUniversalConstants();
...
% [~,SigmaMax]=VoigtProfile(AlphaD,AlphaL,LineCenterRef,LineCenterRef,LStrength);
% Sigma = Sigma./SigmaMax;
Sigma = Sigma./1e4; % Converting from cm^2 to m^2 
end

function [Const] = DefineUniversalConstants()
%
%
%
%
...
Const.M   = 31.9988E-3./6.022E23;    % Mass of a single oxygen molecule
...
Voigt  = real(fadf(((Nu-Nu0) + 1j.*AlphaL)./SigmaD./sqrt(2)));
Sigma = (S./AlphaD).*(sqrt(log(2)./pi)).*Voigt;
end
