...
function [S] = BuildSpectraDerivative(PCASpectra,T,P,Lambda,Options)
%
...
for m=1:1:PC2Use
    ...
    PolyWeights2(m,:) = polyvalnDerivative(PolyFitParams{m,1},[NormalizePolyInputs(TempRebuild,MeanT,SigmaT),NormalizePolyInputs(PressRebuild,MeanP,SigmaP)]);
end
...
% Adding chain rule components to derivatives
RebuildPoly = RebuildPoly./SigmaT;

end
% polyvaln: evaluates a polynomial model as a function of its variables
function ypred = polyvalnDerivative(polymodel,indepvar,DerVar)
%
% arguments: (input)
%  DerVar   - variable to take the derivativve of the model of
...
% Multiplying the overall coefficient by the polynomial order
polymodel.Coefficients = polymodel.Coefficients .* polymodel.ModelTerms(:,DerVar)';
% Redusing the polynomial order of all elements by 1
polymodel.ModelTerms(:,DerVar) = polymodel.ModelTerms(:,DerVar) - 1;
% Removing terms where the polynomial order is less than 0
polymodel.Coefficients(polymodel.ModelTerms(:,DerVar)<0) = [];
polymodel.ModelTerms(polymodel.ModelTerms(:,DerVar)<0,:) = [];
